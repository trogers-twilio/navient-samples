/*
    ...
    name: voicemail-menu
    path: /voice-queue/voicemail-menu
    private: CHECKED
...
exports.handler = function(context, event, callback) {
  ...
  async function createVoicemail(result, taskInfo) {
    ...
    const originalTaskAttributes = JSON.parse(taskInfo.originalTaskData.attributes);
    ...
    const taskAttributes = {
      ...
      conversations: {
        ...
        segment_link: result.recordingUrl
      }
    };
  ...
  async function callModify(sid) {
    let redirect = domain + "/voice-queue/voicemail-menu?mode=main";

    try {
      await client
        .calls(sid)
        .update({ method: "POST", url: redirect });
      console.log("callModify Success");
    } catch (error) {
  ...
  switch (mode) {
    ...
    case "main":
      ...
      twiml.record({
        action: domain + "/voice-queue/voicemail-menu?mode=success&callsid=" + callSid,
        transcribeCallback:
          domain + "/voice-queue/voicemail-menu?mode=submitVoicemail&callsid=" + callSid,
        method: "GET",
        // maxLength: 20,
        playBeep: "true",
        transcribe: true,
